...
def GRF_prewav_to_hat_precomputed(Lmatrices, theta, kappa, alpha, level, y):
    ...
    :param Lmatrices: list of sparse matrices
    :param theta: float
    :param kappa: float
    :param alpha: float
    :param level: int
...
def block_chol_M_precomputed(Lmatrices, l, y):
    """
    Computes action of block Cholesky factors on parameter vector y
    :param Lmatrices: list of sparse matrices
    :param l: int
    :param y: numpy array
    :return: numpy array
...
def wav_to_hat(level):
    ...
    for kk in range(N - 2):
        ...
        J[3 * k_tilde : 3 * (k_tilde + 1)] = array(
            [-1 + 2 * k_tilde, 2 * k_tilde, 2 * k_tilde + 1]
        )
    ...
    for kk in range(N - 2):
        ...
        I[3 * N + 5 * k_tilde : 3 * N + 5 * (k_tilde + 1)] = N + array(
            [k_tilde, k_tilde, k_tilde, k_tilde, k_tilde]
        )
        J[3 * N + 5 * k_tilde : 3 * N + 5 * (k_tilde + 1)] = array(
            [
                -1 + 2 * k_tilde,
                2 * k_tilde,
                1 + 2 * k_tilde,
...
def prewav_to_hat_coeffs(level):
    """
    Transformation from prewavelet to hat function basis
    ...
    for kk in range(M):
        ...
        I[3 * M + 5 * kk + 2 : 3 * M + 5 * (kk + 1)] = array(
            [2 * kk + 1, 2 * kk + 1, 2 * kk + 1]
        )
        J[3 * M + 5 * kk + 2 : 3 * M + 5 * (kk + 1)] = M + mod(
            array(
                [
                    kk,
                    kk + 1,
...
def compute_chol_M(l):
    ...
    for k in range(l + 1):
        ...
        if not os.path.exists(L_matrix_dir):
            os.mkdir(L_matrix_dir)
        ...
        if k == 0:
            ...
            h = 2 ** (-(k + 2))
            M = (
                array([[4, 1, 0, 1], [1, 4, 1, 0], [0, 1, 4, 1], [1, 0, 1, 4]])
                * h
                / 6.0
            )
            L = linalg.cholesky(M)
        ...
        if k >= 2:
            ...
            for kk in range(N - 4):
                ...
                I[5 * k_tilde : 5 * (k_tilde + 1)] = array(
                    [k_tilde, k_tilde, k_tilde, k_tilde, k_tilde]
                )
                J[5 * k_tilde : 5 * (k_tilde + 1)] = array(
                    [-2 + k_tilde, -1 + k_tilde, k_tilde, k_tilde + 1, k_tilde + 2]
                )
...
def hat_to_prewav_new(level):
    """
    Computes transformation from hat function basis
    to prewavelet basis
