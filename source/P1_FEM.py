...
element_mass_fct = lambda h: array([[2,1],[1,2]])*h/6.

# basis functions (on reference interval [-1,1])
w = [lambda t: (1.-t)/2., lambda t: (1. + t)/2.]
...
def element_stiffness_quadrature(mesh, coeff, k):
    ...
    for i in range(2):
        for j in range(2):
            Aloc[i,j] = integrate.quad(lambda x: coeff(((x0+x1)+x*h)/2.)*dw[i](x)*dw[j](x),-1,1)[0] *2./h
    return Aloc
...
def element_stiffness_pwlinear_logcoeff(mesh, coeff, k):
    ...
    x1 = mesh.nodes[mesh.elements[k][1]]
    h = x1-x0
    return exp( (coeff[k] + coeff[k+1])/2. )*sinh( (-coeff[k]+coeff[k+1])/2. )*2./(-coeff[k]+coeff[k+1]) *element_stiffness_fct(h)    
...
def element_mass_quadrature(mesh, coeff, k):
    ...
    for i in range(2):
        for j in range(2):
            Mloc[i,j] = integrate.quad(lambda x: coeff(((x0+x1)+x*h)/2.)*w[i](x)*w[j](x),-1,1)[0] * h / 2.
    return Mloc
...
def assemble_rhs(mesh,fct=None):
    ...
    if fct == None:
        return ones(dofs) * diff(mesh.nodes)/2.
    B = zeros(dofs)
    for k in range(N):
        x0 = mesh.nodes[mesh.elements[k][0]]
        x1 = mesh.nodes[mesh.elements[k][1]]
        h = x1-x0
        for j in r_[:2]:
            B[k+j] += integrate.quad(lambda x: fct(((x0+x1)+x*h)/2.)*w[j](x), -1, 1)[0] * h/2
    return B
...
def assemble_rhs_periodicBC(mesh,fct=None):
    ...
    for k in range(N):
        ...
        h = x1-x0
        for j in r_[:2]:
            B[mod(k+j,N)] += integrate.quad(lambda x: fct(((x0+x1)+x*h)/2.)*w[j](x), -1, 1)[0] * h/2
    return B
    ...
    for k in range(N):
        ...
        for xx in x[i0:]:
            if (xx <= x1):
                ...
                break
        if i == i0: continue # nothing to do, no pts in element
        i1 = i
        ...
        for j in r_[:3]: y[i0:i1] += w[j](X) * c[2*k+j]
        if stop: break
    return y
...
def L2error(u, mesh, exact):
    ...
    #    x0 = mesh.nodes[mesh.elements[k][0]]
    #    x1 = mesh.nodes[mesh.elements[k][1]]
    err = integrate.quad(lambda x: (eval_fct_pt(u,mesh,x)-exact(x))**2.,mesh.D[0],mesh.D[1])[0]
    err = sqrt(err)
    return err
    ...
    for j in range(len(x)):
        if x[j] == mesh.D[1]:
            ...
            for k in range(len(mesh.elements)):
                if x[j] < mesh.nodes[mesh.elements[k][0]] or x[j] >= mesh.nodes[mesh.elements[k][1]]:
                    continue
                else:
                    i = k
                    break
...
def gen_mesh(D,N,meshtype):
    ...
    if meshtype == "equidistant":
        ...
        nodes = [D[0]+(D[1]-D[0])*(2**k-1)/2.**k for k in r_[:N]]
        nodes.append(D[1])
        elements = [(i,i+1) for i in r_[:N]]
    elif meshtype == "equidistant2": # first element is [0,0.5], rest split into N pieces
        nodes = hstack([D[0],linspace((D[0]+D[1])/2.,D[1],N)])
        elements = [(i,i+1) for i in r_[:N]]
    elif meshtype == "log":
...
if __name__ == "__main__":
    ...
    coeffs = 1 - theta/r_[1:Nc+1]**2.

    # right-hand side
    c=15.
    n=0.
    fct = lambda x: c*x**n
    exact = lambda x: c*(x-x**(n+2.))/((n+1.)*(n+2))
    ...
    if do_cppcompare:
        ...
        u2 = array([0,  6.728451305369703,  2.575020569537857,  2.280359038729515,  1.939866750080319,  1.725156250347434,  1.493474440358523,  1.314144471945334,  1.125431448442045, 0.9676230668894894, 0.8036865986557135, 0.6609122202894996, 0.5137486876239928, 0.3823142818570656, 0.2475453801037013, 0.1250923500794603,                  0])
    ...
    if do_conv:
        ...
        #plt.show()
        plt.figure()
        plt.loglog(2.**r_[1:Nce+3],e_L2,'-o')
        plt.grid(True)
        plt.show()
    ...
    if do_range:
        ...
        for t in [-1,1]:
            coeffs = abar + t*theta*ones(N) / r_[1:N+1]**2.
