...
Script to create and save plots of the error vs. 
the required computational cost of MLQMC-FEM
"""
...
def preprocess_data(res, tau, chi):
    ...
    for rep in res:
        ...
        N = len(rep)
        err.append(np.sqrt(sum([(x - refval) ** 2.0 for x in rep])))
        if N > 1:
            err[-1] /= np.sqrt((N - 1))
...
def plot_values(
    axes, err, work, do_lambdas, lambdas, do_sigmas, sigmas, folder_idx, do_fit, lmin
):
    """
    Create plot of error data points versus work data points
    Input

    :axes: axes instance of figure
    :err: list of float
    :work: list of float
    :do_lambdas: bool
    ...
    if do_lambdas and do_sigmas:
        lstr = (
            "$\lambda =$"
            + str(lambdas[folder_idx])
            + ", $\sigma_0 =$"
            + str(sigmas[folder_idx])
        )
...
def get_labels(folders):
    """
    Outputs values of lambdas and sigmas for given folder names
...
def get_folders(sigmas, lambdas, regularity, genvec_informed):
    ...
    :sigmas: list of float
    :lambdas: list of float
    :regularity: str, possible values are either "smooth" or "rough"
    :genvec_informed: bool
    :return: list of str
    """
    folders = []
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--regularity",
        type=str,
        help="regularity of the log Gaussian input",
        default="rough",
    )
    parser.add_argument(
        "--sigmas",
    ...
    parser.add_argument(
        ...
        nargs=3,
        type=float,
        help="values of lambda",
        default=[0.1, 0.1, 0.1],
    )
    parser.add_argument(
        "--do_sigmas",
        help="true if sigmas should appear in legend",
        action="store_true",
    )
    parser.add_argument(
        "--do_lambdas",
        help="true if lambdas should appear in legend",
        action="store_true",
    )
    parser.add_argument(
        "--genvec_informed",
        help="true if genvecs were informed by random input",
        action="store_true",
    )
    ...
    parser.add_argument(
        "--lmin",
        type=int,
        help="number of data points not considered in the empirical conv rate fit",
        default=4,
    )
    parser.add_argument(
        "--mmax", type=int, help="number of data points in the plot", default=12
    )
    parser.add_argument(
        "--do_fit",
        nargs=3,
        type=int,
        help="1 if legend 0 if not legend",
        default=[1, 1, 1],
    )
    parser.add_argument("--figfile", type=str, default="convergence_logN.pdf")
    parser.add_argument(
        "--do_show",
        help="if true plot will be shown during computation",
        action="store_true",
    )
    ...
    folders = get_folders(
        args.sigmas, args.lambdas, args.regularity, args.genvec_informed
    )
