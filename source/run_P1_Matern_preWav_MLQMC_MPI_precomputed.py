...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--Lmax", type=int, help="maximal level of prewavelet expansion"
    )
    parser.add_argument("--rep", type=int, help="number of random shift")
    parser.add_argument(
        "--alpha", type=int, help="alpha related to smoothness of GRF", default=2.0
    )
    ...
    parser.add_argument(
        "--sigma0", type=float, help="standard deviation of GRF", default=1.0
    )
    parser.add_argument(
        "--lambdaC", type=float, help="correlation length of GRF", default=0.1
    )
    parser.add_argument(
        "--genvec_informed",
    ...
    for l in range(0, Lmax + 1):
        ...
        if l == 0:
            ...
            integrand = integrand_single(
                kappa, theta, alpha, l, Lmatrices, s_leaf, shift
            )
            ...
            if alpha == 4:
                latfile = latfile_dir + "/lat-n%ds%dalpha10.100000.json" % (
                    Nqmc,
                    s_leaf,
                )
            if rank == 0:
            ...
            # instantiate integrand
            integrand = integrand_difference(
                kappa, theta, alpha, l, Lmatrices, s_leaf, s_level, Sused, shift
            )
        ...
        output["result"] = res
        parameters = {"alpha": alpha, "sigma0": sigma0, "lambdaC": lambdaC}
        output["params"] = parameters

        # write output
        st = "logN"
        outfile = os.path.join(
            outdir, "%s_a%d_m%d_s%d.json" % (st, alpha, Lmax, s_leaf)
        )
        if rank == 0:
